...
async def cb_game_select(callback: CallbackQuery, container: Container, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Список игр", callback_data="application_start")]
    ])

    # Отправляем фото игры, если оно существует
    if game.photo_path and Path(game.photo_path).exists():
        ...
        # Удаляем старое сообщение с текстом, если это было редактирование
        try:
