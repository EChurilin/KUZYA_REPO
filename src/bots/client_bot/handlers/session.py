...
async def cb_session_start(callback: CallbackQuery, container: Container, state: FSMContext):
    ...
    if not game_id_str:
        await callback.message.edit_text("Сначала выберите игру.")
        return
...
async def cb_session_finish(callback: CallbackQuery, container: Container, state: FSMContext):
    ...
    if not session_id_str:
        await callback.message.edit_text("У вас нет активной сессии.")
        return
    
    try:
        session_id = uuid.UUID(session_id_str)
    except ValueError:
        await callback.message.edit_text("Ошибка: некорректный идентификатор сессии.")
        return
    ...
    try:
        ...
        await callback.message.edit_text(f"Ошибка при создании заявки: {str(e)}")